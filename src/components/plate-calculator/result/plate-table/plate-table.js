...
const PlateTable = ({
  ...
  return (
    <table className="table-auto w-full">
      ...
      <thead>
        <tr>
          <th className="px-4 py-2">Number of plates</th>
          <th className="px-4 py-2">Weight ({ plates[0]?.weight })</th>
        </tr>
      </thead>
      <tbody>
        {
          plates.map(({ weight, unit, count }, index) => {
            return (
              <tr key={`plate__${weight}${unit}`} className={classnames({ "bg-gray-100": index % 2 !== 0 })}>
                <td className="border px-4 py-2" title="Number of plates">{ count / 2 }</td>
